...
class Argument(object):
    ...
    name - Each argument has a unique name that is used to specify the argument when
    passing a value to it as a keyword argument in the wraprun API.
    ...
    split (default: False) - Specifies whether the argument can split a task MPI
    communicator into several 'colors'. Spitting arguments are always passed as
    an iterable in the API or a comma-separated string on the CLI.

    parser - a dictionary of extra keyword arguments passed to
    argparse.ArgumentParser.add_argument to define help strings, default values,
    and parser actions. In particular, arguments must have an 'action' that
    over-rides the default behavior of argparse such that arguments that do not
    appear on the CLI are given a value of None in the namespace returned by
    argparse.parse_*_args()

    formatter - an optional function that formats the argument value as it needs
    to appear on the CLI to be passed to aprun. IE, this is used to convert
    split-capable aprun arguments from their wraprun form to their aprun form.
    See the 'pes' argument for an example.
    """
...
class ArgumentList(object):
    ...
    This class provides special methods for common operations on sets of wraprun
    API and CLI arguments. In particular, it maintains the order of the added
    arguments and allows lookup by argument name.
    """
